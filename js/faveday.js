...
(function() {
  ...
  FaveDayApp = class FaveDayApp {
    ...
    setupScores() {
      if (typeof demoNotes !== "undefined" && demoNotes !== null) {
        return this.setupDemoUser();
      } else {
        return this.setupLocalFolder();
      }
    }

    setupLocalFolder() {
      var dialog, file, files, folder, fs, j, len, matches, re, txt;
      this.all = [];
      if (window.localStorage.folder == null) {
      ...
      for (j = 0, len = files.length; j < len; j++) {
        ...
        while (true) {
          ...
          this.all.push(new Score(new Date(matches[1]), parseInt(matches[2]), matches[3]));
        }
      }
      console.log(`loaded ${this.all.length} entries`);
      this.all = this.all.sortBy('date', true);
